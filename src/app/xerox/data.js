...
export default function Data() {
  ...
  async function getProducts() {
    try {
      ...
      const response = await fetch('/data/xerox-catalog.json')
      if (response.ok) {
        const data = await response.json()
        setProducts(data)
        setSearching(true)
        localStorage.setItem("xerox", JSON.stringify(data))
      } else {
        ...
        setSearching(true)
        localStorage.setItem("xerox", JSON.stringify(data1.cancel.products))
  ...
  useEffect(() => {
    if (localStorage.getItem("xerox")) {
      setSearching(true)
      setToner(JSON.parse(localStorage.getItem("xerox")))
    }
  }, [products])
